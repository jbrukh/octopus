module Trashable
  ...
  module ClassMethods
    ...
    def with_deleted_scope_sql
      all.table[:trashed_at].eq(nil).to_sql
    end
  end
  ...
  def trash!
    ...
    #
    update_column(:trashed_at, DateTime.now)
  end
  ...
  def recover!
    ...
    # Fixed in Rails 4
    #
    self.class.unscoped.update_all({trashed_at: nil}, id: self.id)
    raw_write_attribute :trashed_at, nil
  end
